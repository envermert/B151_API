...
public class Post01 extends JsonplaceholderBaseUrl {
    ...
    public void post01Map() {
        ...
        //3. Send the request and get the repsonse
        Response response = given(spec).body(expectedData).when().post("{first}");//Serialization= Java Map datasinin Json datasina cevrilmesidir.
        response.prettyPrint();

        //4. Do assertion

        Map<String,Object> actualData=response.as(HashMap.class);//De-Serialization = Json objesinin Java Map objesine cevrilmesi
        assertEquals(201,response.statusCode());
